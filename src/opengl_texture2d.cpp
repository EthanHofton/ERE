...
GLenum opengl_texture2d::get_gl_format(const texture_api::format& format) const {
    switch (format) {
...
GLenum opengl_texture2d::get_gl_internal_format(const texture_api::format& format) const {
    switch (format) {
        case texture_api::format::RED:
            return GL_RED;
        case texture_api::format::RG:
...
GLenum opengl_texture2d::get_gl_filter(const texture_api::filter& filter) const {
    switch (filter) {
...
GLenum opengl_texture2d::get_gl_wrap(const texture_api::wrap& wrap) const {
    switch (wrap) {
...
GLenum opengl_texture2d::get_gl_type(const texture_api::format& format) const {
    switch (format) {
        case texture_api::format::RED:
        case texture_api::format::RG:
        case texture_api::format::RGB:
